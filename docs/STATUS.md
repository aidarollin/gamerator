...
## 2026-09-10 (later) — Three input mediums, both halves of the product, DS re-synced

Zul: *"add more mediums for user to include in the prompt/input (game link,
picture, docs for the game flow/description). and make more game templates
available to be generated but with pandai Design System(or a touch of Pandai),
and then fetch Pandai Design System 1.5."*

Three asks, three commits, each verified by driving the thing rather than by
reading the code.

### 1. Pandai DS 1.5 — re-synced, and the answer was "nothing changed"

The file has moved: Primitives 542→545, Product 90→94, and three collections
that did not exist at the last sync (`Subjects (A–E)` and `Subjects (G–S)`,
which carry per-subject theming as MODES, and `Platform (Mobile)`). **None of it
reaches the Semantic layer this build reads.**

Proved with a digest computed identically inside Figma and against the committed
JSON — `786f6dbc` both sides, 366 tokens — rather than by eyeballing a diff.
`npm run tokens` regenerated byte-identically, so neither generated file appears
in the commit. `sync-tokens.md` carries that step now, plus what the extractor
skips (`JDP/`, 20 variables, previously dropped in silence).

### 2. A Pandai skin, and the first version was bad

`theme.skin: "pandai"` renders any arcade game through the DS token ramp. **The
first version was photographed and it was pink sky over pink pipes over pink
ground** — a single subject ramp painting everything, which is exactly the
"diagram of a game" that arcade scenes were invented to escape. It would have
shipped as a feature that makes games worse.

`dsPalette` now gives the two families `palettes.ts` insists on, both from the
token layer: the subject ramp is the identity and paints what you hit, and
Pandai primary green paints the sky. **More** on-brand rather than less.

### 3. The five learning templates are askable

They were built first, are fully tested, are made entirely of DS tokens, and
were reachable only from `/play/preview`. So "a quiz about photosynthesis" got a
flyer, and the five games in this repo that were ALREADY pure Pandai were the
ones nobody could ask for. `catalogue.ts` has a fourth disposition; the gallery
shows all fifteen; one refusal became a game.

The free path differs between the two families and the page says so: the tuner
DERIVES arcade physics from your words, but a free learning spec is a canned
fixture — right template, somebody else's content.

### 4. Link, picture, notes

All three feed the **router**, not just the model. Before this, pasting a design
document and typing "make this" was a coin flip: the document reached the model
but was invisible to the code choosing which engine the model was asked about.

`/create` is GET **and** POST — text briefs still redirect to a shareable URL,
uploads are answered inline and told they have no link.

### What driving the form found that 374 tests did not

1. **It threw away what was typed.** A POST leaves the address bar alone, so the
   form re-rendered from empty `searchParams` and a pasted design document was
   simply gone. Same family as the `/create` box being eaten by the game's key
   handler — the product destroying work somebody just did.
2. **The first real brief was rejected as unplayable.** The maze seed came from
   `hash(prompt)`, and about one board in fifty comes out trivial. That works
   forty-nine times; the fiftieth person gets "the game that came back was not
   playable" for a completely reasonable request. The seed is searched now.

Both were found on the first honest end-to-end run, and neither was reachable
from a unit test.
